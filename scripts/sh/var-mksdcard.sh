...
function format_android
{
	...
	if [[ "${dynamic_img}" = false ]]; then
		...
		out/host/linux-x86/bin/make_f2fs -l metadata -f -g android ${node}${part}14
		blue_underlined_bold_echo "Formatting userdata partition"
		out/host/linux-x86/bin/make_f2fs -l userdata -f -g android ${node}${part}20
	else
		blue_underlined_bold_echo "Erasing presistdata partition"
		dd if=/dev/zero of=${node}${part}13 bs=1M count=${PRESISTDATA_SIZE} conv=fsync,nocreat

		blue_underlined_bold_echo "Erasing fbmisc partition"
		dd if=/dev/zero of=${node}${part}16 bs=1M count=${FBMISC_SIZE} conv=fsync,nocreat

		blue_underlined_bold_echo "Erasing misc partition"
		dd if=/dev/zero of=${node}${part}11 bs=1M count=${MISC_SIZE} conv=fsync,nocreat

		blue_underlined_bold_echo "Formatting metadata partition"
		out/host/linux-x86/bin/make_f2fs -l metadata -f -g android ${node}${part}12
